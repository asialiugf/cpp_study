...
class PeriodBase {
   public:
    ...
    static int* period;          //  周期，以秒计， 1(1S) 15(15S)  60(1F) 300(5F)
    static int* period_index;    // 周期的 数组下标
    static char period_name[4];  // 周期名：
    ...
    PeriodBase() {
        std::cout << "PeriodBase construtor!" << std::endl;
        // unit = nullptr;  不能这样搞！！
    }
...
template <int... N>
class FutureSerial : public PeriodBase<N...> {
   public:
    template <int N1>
    int getone() {
        return N1;
    ...
    FutureSerial() {
        ...
        if constexpr (0 == sizeof...(N)) {
            ...
            std::cout << " N must less then 2 !" << std::endl;
        }

        if (PeriodBase<N...>::unit == nullptr) {
            std::cout << " unit is nullptr !!" << std::endl;
            if (f == -1 && s == -1) {
                std::cout << "Future<>  Please!!!!!!!!" << std::endl;
            } else if (f == -1) {
                std::cout << "Future<" << s << "> Please !!!!!!!!" << std::endl;
            } else {
                std::cout << "Future<" << f << "," << s << "> Please !!!!!!!!" << std::endl;
            }
        };
    ...
    FutureSerial(double value) {
        ...
        this->data[this->unit->index] = value;
    ...
    */
    ...
    FutureSerial& operator=(double x) {
        std::cout << "赋值\n";
        this->data[this->unit->index] = x;
        return *this;
    }
    FutureSerial& operator=(const FutureSerial& p) {
        std::cout << "赋值 p\n";
        this->data[this->unit->index] = p.data[this->unit->index];
        return *this;
    }
...
template <int... N>
class Future : public PeriodBase<N...> {
   public:
    Future(Base* base, std::string s, int period) {
        ...
        // todo m n M N
        /*
        Future<N...>::unit = b->unit_array_[0][0];
        Future<N...>::index = &b->unit_array_[0][0]->index;
        Future<N...>::period = &b->unit_array_[0][0]->period;
        Future<N...>::period_index = &b->unit_array_[0][0]->period_index;
        Future<N...>::O = b->unit_array_[0][0]->O;
        Future<N...>::H = b->unit_array_[0][0]->H;
        Future<N...>::L = b->unit_array_[0][0]->L;
        Future<N...>::C = b->unit_array_[0][0]->C;
        Future<N...>::V = b->unit_array_[0][0]->V;
...
int* PeriodBase<N...>::index;  // 然后在这里进行初始化。
template <int... N>            // 这里很神奇啊！！！！
int* PeriodBase<N...>::period;
template <int... N>  // 这里很神奇啊！！！！
int* PeriodBase<N...>::period_index;
template <int... N>  // 这里很神奇啊！！！！
Unit* PeriodBase<N...>::unit = nullptr;
template <int... N>  // 这里很神奇啊！！！！
int* PeriodBase<N...>::ohlcv_len;
template <int... N>  // 这里很神奇啊！！！！
double* PeriodBase<N...>::O = nullptr;
template <int... N>  // 这里很神奇啊！！！！
double* PeriodBase<N...>::H = nullptr;
template <int... N>  // 这里很神奇啊！！！！
double* PeriodBase<N...>::L = nullptr;
template <int... N>  // 这里很神奇啊！！！！
double* PeriodBase<N...>::C = nullptr;
template <int... N>  // 这里很神奇啊！！！！
double* PeriodBase<N...>::V = nullptr;

/*
template <int... N>                                   // 这里很神奇啊！！！！
std::array<double, 1000000> PeriodBase<N...>::o;      // OHLCV是某一组交易基础数据。它由M、N二者一起才能唯一确定。
template <int... N>                                   // 这里很神奇啊！！！！
std::array<double, 1000000> PeriodBase<N...>::h;      // 这里的数组的长度应该怎么搞？？？？ 固定吗?
template <int... N>                                   // 这里很神奇啊！！！！
std::array<double, 1000000> PeriodBase<N...>::l;      // 这里的数组的长度应该怎么搞？？？？ 固定吗?
template <int... N>                                   // 这里很神奇啊！！！！
std::array<double, 1000000> PeriodBase<N...>::c;      // 这里的数组的长度应该怎么搞？？？？ 固定吗?
template <int... N>                                   // 这里很神奇啊！！！！
std::array<double, 1000000> PeriodBase<N...>::v;      // 这里的数组的长度应该怎么搞？？？？ 固定吗?
*/

template <int... N>                                   // M是指future, N是指1F 5F 等周期
Signal<void()> PeriodBase<N...>::future_fun_list[1];  // 每一个<M,N>对，有一个list,里面可能有，多个 macd() 多个kdj() .....

template <int... N>
class MACD : public PeriodBase<N...> {
   public:
    ...
    MACD() {
        std::cout << "MACD  constructor begin ! !!!!" << std::endl;
        if (PeriodBase<N...>::unit == nullptr) {
            std::cout << " Future Unit neet to be Init !!!!" << std::endl;
            exit(-1);
        } else {
            std::cout << " MACD contructor  unit is not nullptr !!" << std::endl;
        };
        ...
        std::cout << "macd constructor !!! -- 1 :  " << 5 << std::endl;     // N
        Future<N...>::future_fun_list[0].Connect(this, &MACD<N...>::macd);  //
        std::cout << "macd constructor !!! -- 2 :  " << 5 << std::endl;     // N
    }
...
void test_one_x(Base* bb) {
    ...
    std::cout << " in test_one_x !!  001 ------------------- " << std::endl;
    ...
    std::cout << " in test_one_x !!  002 ------------------- " << std::endl;

    for (int i = 0; i < 10; i++) {
        std::cout << " in test_one_x !!  003 ------------------- " << std::endl;
        std::cout << " in test_one_x !!  003 ------------------- " << *Future<10>::index << std::endl;
        std::cout << " in test_one_x !!  003 ------------------- " << (*one1.period) << std::endl;
        std::cout << " in test_one_x !!  003 ------------------- " << one1.index << std::endl;
        (*one1.period) = i;
        (*one1.index) = i;
        std::cout << " in test_one_x !!  004 ------------------- " << std::endl;
        one1.O[i] = sqrt(i * 17);
        std::cout << " in test_one_x !!  005 ------------------- " << std::endl;
...
#define LOCAL_DATA false
    ...
    fuMap["zz1805"] = {5, BAR_TK, BAR_1S, BAR_1F, BAR_2H};
    fuMap["ag1912"] = {6, BAR_TK, BAR_1S, BAR_1F, BAR_2H};
    fuMap["fu2305"] = {7, BAR_TK, BAR_1S, BAR_1F, BAR_2H};
    fuMap["au2306"] = {8, BAR_TK, BAR_1S, BAR_1F, BAR_2H};

    bb.FutureInit(fuMap, LOCAL_DATA);

    Future<au2306, 4> f01(&bb, "au2306", 4);
    Future<au2306, 500> f02(&bb, "au2306", 500);
    f01.unit->index = 51245;  // Future 会根据模板参数，分别初始化。 这里初始化了两组，分别是 <4> <500>
    f02.unit->index = 9999;   // 每组都有自己独立的。静态成员空间。
    std::cout << "here !!" << std::endl;  // 51425
    std::cout << "here !!" << std::endl;  // 51425
    std::cout << "here !!" << std::endl;  // 51425
    std::cout << "here !!" << std::endl;  // 51425
    MACD<au2306, 4> x;
    ...
    */
    ...
    bb.onBars([](int e) {
        std::cout << " onBars data: -----------------------------------" << e << std::endl;

        // 下面的这种static定义的变量，均与 instrumentID 和 period相关。
        //  todo !!
        // 但如果是onTick() 里面，如何知道是哪个instrumentID的 Tick呢？？

        /* todo  查一下！ 没有如下初始化，出现 Segmentation fault !!   */
        /*
        Future<> f1(&bb);
        Future<15> f2(&bb);
        Future<au2306, 15> f3(&bb);
        Future<au2306, 30> f4(&bb);
        Future<au2306, 60> f5(&bb);
        Future<au2306, 15> f6(&bb);
        Future<au2306, 7> f7(&bb);
        Future<au2306, 19> f8(&bb);
        Future<au2306, 21> f9(&bb);
        */

        static MACD<> m0_0;
        std::cout << " from onBars data: ----------------------------------- 111" << e << std::endl;
        Future<15> k(&bb);
