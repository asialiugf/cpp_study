    ...
    while (running) {
        ...
        */
        if (lounger == 1) {
            Entry cur = q->pop();
            auto  ts  = now1();
            cout << "tid: " << cur.tid << " val: " << cur.val << " latency: " << (ts - cur.ts)
                 << " cycles" << endl;
        } else if (lounger == 2) {
            Entry cur;
            // we can't check running flag here because we must commit the idx we got before exiting
            // so program would hang if no writer exits, uncomment the empty check above to avoid
            // this issue
            while (!q->tryPop(cur))
                ;
            auto ts = now1();
            ...
            while ((data = q->getReadable(idx)) == nullptr)
                ;
            auto ts = now1();
            // cout << "----- here    tid: " << data->tid << " val: " << data->val << " latency: "
            // << (ts - data->ts) << " cycles"
            cout << "----- here    tid: " << data->tid << " val: " << data->val
                 << " latency: " << data->ts - ts << " cycles" << endl;
            q->commitRead(idx);
        }
    }
}
