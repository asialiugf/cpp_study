...
int deal_accept(int sockfd) {
    ...
    perror("kkkkk!!");
    printf("error !!!!  connfd = %d\n", connfd);
    return connfd;
}
int deal_accept2(int sock) {
    while (1) {
...
/* 处理epoll的返回结果 */
void epoll_process(int epollfd, struct epoll_event *events, int number, int sockfd, int epoll_type,
                   int block_type) {
    struct sockaddr_in client_addr;
    socklen_t          client_addrlen;
    int                newfd, connfd;
...
int create_socket(const char *ip, const int port_number) {
    ...
    if (inet_pton(PF_INET, ip, &server_addr.sin_addr) == -1)
        err_exit(const_cast<char *>("inet_pton() error"));

    if ((sockfd = socket(PF_INET, SOCK_STREAM, 0)) == -1)
        err_exit(const_cast<char *>("socket() error"));

    // set_nonblock(sockfd);

    /* 设置复用socket地址 */
    if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(reuse)) == -1)
        err_exit(const_cast<char *>("setsockopt() error"));

    if (bind(sockfd, (struct sockaddr *)&server_addr, sizeof(server_addr)) == -1)
        err_exit(const_cast<char *>("bind() error"));

    if (listen(sockfd, 5) == -1)
        err_exit(const_cast<char *>("listen() error"));
...
int main(int argc, const char *argv[]) {
    ...
    int sockfd, number;
    sockfd = create_socket(argv[ 1 ], atoi(argv[ 2 ]));  // listen
    // set_nonblock(sockfd);

    /* test!!! */
    int conn = deal_accept(sockfd);
    // int conn = deal_accept2(sockfd);
     printf("haha ============================= test \n");
    /* test!!! */
